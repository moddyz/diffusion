...
class Diffusion:
    ...
    def __init__(self, num_steps):
        ...
        self._sqrt_recip_alphas_cumprod_minus_one = torch.sqrt(
            1.0 / self._alphas_cumprod - 1.0
        )

        # Pre-compute posterior mean co-efficients
        self._alphas_cumprod_prev = F.pad(self._alphas_cumprod[:-1], (1, 0), value=1.0)
        self._posterior_mean_start_coeff = (
            self._betas
            * torch.sqrt(self._alphas_cumprod_prev)
            / (1.0 - self._alphas_cumprod)
        )
        self._posterior_mean_t_coeff = (
            torch.sqrt(self._alphas)
    ...
    def add_noise(
        ...
        sqrt_alphas_cumprods_t = self._extract_and_reshape(
            self._sqrt_alphas_cumprod, time_step
        )
    ...
    def posterior_terms(
        ...
        posterior_mean_start_coeff = self._extract_and_reshape(
            self._posterior_mean_start_coeff, time_step
        )
        posterior_mean_t_coeff = self._extract_and_reshape(
            self._posterior_mean_t_coeff, time_step
        )
        ...
        posterior_variance = self._extract_and_reshape(
            self._posterior_variance, time_step
        )
